% Second level analysis with response type as covariate
% Pragmatic = 1
% Logical = -1
% Neither = 0
...
second_level_path = 'C:\Users\TalT\Dropbox\Documents\SI_MRI_analysis\second_level_analysis_covariate';
...
covariate = zeros(1, length(subjects));
% Find the indices of pragmatic and logical participants
...
% Change covariate vector according to participants' responses
covariate(pragmatic_indices) = 1;
covariate(logical_indices) = 2;

% SPM inputs
nrun = length(all_subjects_contrasts{1}); % one run for each contrast
jobfile = {'second_level_covariate_job.m'};
...
for crun = 1:nrun
    ...
    inputs{3, crun} = covariate'; % Factorial design specification: Vector - cfg_entry
